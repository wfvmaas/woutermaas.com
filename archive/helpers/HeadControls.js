...
const HeadControls = (function () {
  ...
  let _gl = null,
    _cv = null,
    _videoTexture = null,
    _videoTransformMat2 = null,
    _glHeadSearchDrawShaderProgram = null
  let _headSearchUniformXys = null,
    _headSearchUniformVideoTransformMat2 = null
  ...
  function init_headSearchDraw() {
    ...
    void main(void) {\n\
      ...
      vec2 pos = vUV * 2.0 - vec2(1.0, 1.0);\n\
      vec2 isInside = step(uxys.xy - uxys.z * vec2(1.0, 3.0), pos);\n\
      isInside *= step(pos, uxys.xy + uxys.z * vec2(1.0, 3.0));\n\
      vec2 borderCheck = abs(pos - uxys.xy) - uxys.z * vec2(0.95, 2.95);\n\
      float borderAlpha = max(borderCheck.x, borderCheck.y);\n\
      borderAlpha = step(0.0, borderAlpha) * step(borderAlpha, 0.02); // Thin border\n\
      vec3 borderColor = vec3(0.0, 1.0, 0.0); // Terminal green color\n\
      vec3 finalColor = mix(colorVideo, borderColor, 0.5 * borderAlpha); // 50% opacity\n\
    ...
    _headSearchUniformVideoTransformMat2 = _gl.getUniformLocation(
      _glHeadSearchDrawShaderProgram,
      "videoTransformMat2"
    )
  ...
  function compute_cameraMove(detectState) {
    if (
      _state.isDetected &&
      detectState.detected < _settings.detectionThreshold - _settings.detectionHysteresis
    ) {
      // DETECTION LOST
    ...
    _returnValue.dRx =
      dt *
      compute_delta(
        _state.restHeadPosition.rx,
        detectState.rx,
        _settings.tol.rx,
        _settings.sensibility.rx
      )
    _returnValue.dRy =
      dt *
      compute_delta(
        _state.restHeadPosition.ry,
        detectState.ry,
        _settings.tol.ry,
        _settings.sensibility.ry
      )
    _returnValue.dZ =
      dt *
      compute_delta(
        _state.restHeadPosition.s,
        detectState.s,
        _settings.tol.s,
        _settings.sensibility.s
      )
  ...
  const that = {
    init: function (spec) {
      ...
      _disableRestPosition =
        typeof spec.disableRestPosition === "undefined" ? false : spec.disableRestPosition
